...
class NliProcessor(DataProcessor):
    ...
    def get_train_examples_for_split(self, data_dir, split_num):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "train_" + str(split_num) + ".tsv"))
        examples = []
    ...
    def get_dev_examples_for_split(self, data_dir, split_num):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "dev_"+str(split_num)+".tsv"))
        examples = []
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % ("test", i)
            text_a = line[0]
...
MODEL_CLASSES = {
    ...
    "custom": (BertConfig, BertWithLinearLayer, BertTokenizer),
}
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type != "distilbert":
                inputs["token_type_ids"] = (
                    batch[2] if args.model_type in ["bert", "custom"] else None
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results = eval_train_or_test(args, model, tokenizer)
            ...
            if args.max_steps > 0 and global_step > args.max_steps:
                ...
                break
        eval_train_or_test(args, model, tokenizer, epoch=epoch_num)
        eval_train_or_test(args, model, tokenizer, eval=False, epoch=epoch_num)
        epoch_num += 1
...
def eval_train_or_test(args, model, tokenizer, prefix="", eval=True, epoch=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if epoch is None:
            output_eval_file = os.path.join(eval_output_dir, prefix, tord+"_eval_results_split_" + str(args.split_num) + ".txt")
        else:
            output_eval_file = os.path.join(eval_output_dir, prefix, tord + "_eval_results_split_" + str(args.split_num) + "_epoch_" + str(epoch) + ".txt")
        ...
        if eval and epoch is None:
            with open(os.path.join(eval_output_dir, "preds_and_labels_split_"+str(args.split_num)+".csv"), "wt") as out_file:
                tsv_writer = csv.writer(out_file, delimiter='\t')
                tsv_writer.writerows([["prediction", "label"]] + preds_labels)
        elif eval and epoch is not None:
            with open(os.path.join(eval_output_dir, "preds_and_labels_split_"+str(args.split_num)+ "epoch_"+ str(epoch) + ".csv"), "wt") as out_file:
                tsv_writer = csv.writer(out_file, delimiter='\t')
                tsv_writer.writerows([["prediction", "label"]] + preds_labels)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = (
            # change the first thing back to get_test_examples
            processor.get_dev_examples_for_split(args.data_dir, args.split_num) if evaluate else processor.get_train_examples_for_split(args.data_dir, args.split_num)
        )
...
def plot_curves(args):
    ...
    plt.plot([i for i in range(len(all_dev_acc))], all_dev_acc, color='blue',  marker='o', label='dev')
    plt.plot([i for i in range(len(all_train_acc))], all_train_acc, color='red', marker='o', label='train')
    plt.xticks([i for i in range(len(all_train_acc))])
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Accuracy per epoch")
    plt.xticks([i for i in range(len(all_train_losses))])
    plt.legend()
    plt.savefig(output_eval_file_acc + ".png")
    plt.close()
    plt.plot([i for i in range(len(all_dev_losses))], all_dev_losses, color='blue', marker='o', label='dev')
    plt.plot([i for i in range(len(all_train_losses))], all_train_losses, color='red', marker='o', label='train')
    plt.xticks([i for i in range(len(all_train_losses))])
    plt.legend()
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Cross validation split number",
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,
